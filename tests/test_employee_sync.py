...
@pytest.mark.parametrize("case_path", [
    ...
    "tests/data_case_04_missing_in_gtn"
])
def test_employees_in_payrun_exist_in_gtn(case_path):
    ...
    payrun_df = pd.read_excel(payrun_path, sheet_name="Payrun file", engine='openpyxl')

    gtn_ids = extract_employee_ids(gtn_df, "employee_id")
    payrun_ids = extract_employee_ids(payrun_df, "Employee ID")

    missing_in_gtn, _ = compare_employees(gtn_ids, payrun_ids)

    assert not missing_in_gtn, f"The following Employee IDs are in Payrun but missing in GTN: {missing_in_gtn}"
